...
void Client::onNewDevice(INDI::BaseDevice device)
{
    ...
    if (device.getDeviceName() == name) {
        ...
        ccd = device;
        device.watchProperty(PROPERTY_CONNECTION.c_str(), std::bind(&Client::watchConnection, this, _1),
                             INDI::BaseDevice::WATCH_NEW);
        device.watchProperty(PROPERTY_BLOB.c_str(), std::bind(&Client::watchBlob, this, _1),
...
{
    std::thread::id a;
    std::cout << std::format("[{} {:%X}] {}", to_string(std::this_thread::get_id()), std::chrono::system_clock::now(),
                             message)
              << std::endl;
}
