...
FilenameGenerator::FilenameGenerator(const std::string &dir, const std::string &tag)
: dir{dir}, tag{tag}
{
    if (!fs::exists(dir))
    {
...
{
    ++frame;
    const auto fileName = std::format("{}_{}_{:03}_{:03}.{}", tag, postTag, batch, frame, ext);
    const auto result = fs::path(dir) / fileName;
    if (fs::exists(result))
    {
        throw std::runtime_error(std::format("File {} already exists", result.string()));
    }
    return result;
}
